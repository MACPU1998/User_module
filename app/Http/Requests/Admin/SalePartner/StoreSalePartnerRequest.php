...
class StoreSalePartnerRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            ...
            'mobile' => [
                ...
                Rule::unique('sale_partners','mobile')->where(function ($query) { $query->whereNull('deleted_at');}),
            ],
            //'required|numeric|ir_mobile:zero|unique:users,mobile',
            'phone' =>[
                'nullable',"ir_phone_with_code",
                Rule::unique('sale_partners','phone')->where(function ($query) { $query->whereNull('deleted_at');}),
            ],
            //'required|numeric|ir_phone_with_code|unique:users,phone',
            'national_code' => [
                'required',"ir_national_code",
                Rule::unique('sale_partners','national_code')->where(function ($query) { $query->whereNull('deleted_at');}),
            ],
            ...
            'contract_picture' => 'nullable|file|mimes:jpg,jpeg,png,pdf|max:2048',
            'guarantees_picture' => 'nullable|file|mimes:jpg,jpeg,png,pdf|max:2048',
