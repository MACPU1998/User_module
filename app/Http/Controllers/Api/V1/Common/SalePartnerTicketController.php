...
class SalePartnerTicketController extends Controller
{
    ...
    public function ticketRegister(TicketRequest $request)
    {
        ...
        try {
            $ticket_data = [
                ...
                "creator_id"=>auth()->guard("salePartner")->user()->id,
    ...
    {
        try {
            $tickets =Ticket::where("creator_type",SalePartner::class)->where("creator_id",auth()->guard("salePartner")->user()->id)->get();
    ...
    {
        try {
            $ticket = Ticket::where("creator_type",SalePartner::class)->where("creator_id",auth()->guard("salePartner")->user()->id)->first();
            $ticket_detail = [
                "id"=>$ticket->id,
                "title"=>$ticket->title,
    ...
    public function getTicketMessageAttachments($ticket_message_id)
    {
        try {
            ...
            $ticket_message_attachments = $ticket_message_attachments->map(function($item){
                return [
                    "file_name"=>$item->file,
                    "file_url"=>route("api.v1.sale_partner.tickets.attachment.download",encrypt($item->id)),
                ];
            });
