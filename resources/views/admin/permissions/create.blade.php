...
<div class="card mb-4">
    ...
    <form class="card-body" method="POST" action="{{route('admin.admins_management.permissions.store')}}" enctype="multipart/form-data">
        @csrf

      <div class="row">

        <x-admin.form.text required="true" sectionClass="col-md-6" label="{{__('general.permission_name')}}" name="name" value="{{old('name')}}"/>

        <x-admin.form.text required="true" sectionClass="col-md-6" label="{{__('general.slug')}}" name="slug" value="{{old('slug')}}"/>

        <x-admin.form.select required="false" sectionClass="col-md-6" label="{{__('general.parent')}}" name="parent" :items="$permissions" selectedItem="{{old('parent')}}" translate="true" translateFile="permissions"/>
